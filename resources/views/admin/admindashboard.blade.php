...
<div class="container py-5">
    ...
    <div class="row g-3 g-md-4 mb-5 justify-content-center" id="stats-section">
        <!--filled dynamically code downwards-->
    </div>

    <!--showing property in a list-->
...
const stats = {
    ...
    houses: properties.filter(p => p.property_type === 'House').length,
    apartments: properties.filter(p => p.property_type === 'Apartment').length,
    villas: properties.filter(p => p.property_type === 'Villa').length,
    avgRent: Math.round(properties.reduce((a, b) => a + b.monthly_rent, 0) / properties.length),
    cities: new Set(properties.map(p => p.city)).size
};

//then showing all stats in card form
document.getElementById('stats-section').innerHTML = `
    <div class="col-6 col-sm-4 col-md-4 col-lg-2">
        <div class="stat-card gradient-blue text-center text-white p-4 rounded-4 shadow-sm h-100">
    ...
    <div class="col-6 col-sm-4 col-md-4 col-lg-2">
        <div class="stat-card gradient-green text-center text-white p-4 rounded-4 shadow-sm h-100">
            <i class="bi bi-house-fill fs-1 mb-2"></i>
            <h4 class="fw-bold mb-0">${stats.houses}</h4>
            <small>Houses</small>
        </div>
    </div>
    <div class="col-6 col-sm-4 col-md-4 col-lg-2">
        <div class="stat-card gradient-cyan text-center text-white p-4 rounded-4 shadow-sm h-100">
            <i class="bi bi-building fs-1 mb-2"></i>
            <h4 class="fw-bold mb-0">${stats.apartments}</h4>
            <small>Apartments</small>
        </div>
    </div>
    <div class="col-6 col-sm-4 col-md-4 col-lg-2">
        <div class="stat-card gradient-pink text-center text-white p-4 rounded-4 shadow-sm h-100">
            <i class="bi bi-house-heart fs-1 mb-2"></i>
            <h4 class="fw-bold mb-0">${stats.villas}</h4>
            <small>Villas</small>
        </div>
    </div>
    <div class="col-6 col-sm-4 col-md-4 col-lg-2">
        <div class="stat-card gradient-orange text-center text-white p-4 rounded-4 shadow-sm h-100">
            <i class="bi bi-cash-stack fs-1 mb-2"></i>
            <h4 class="fw-bold mb-0">$${stats.avgRent}</h4>
            <small>Avg Rent</small>
        </div>
    </div>
    <div class="col-6 col-sm-4 col-md-4 col-lg-2">
        <div class="stat-card gradient-gray text-center text-white p-4 rounded-4 shadow-sm h-100">
...
// filling rows based on property.json
const tbody = document.getElementById('property-table-body');
properties.forEach(p => {
    tbody.innerHTML += `
        <tr id="property-${p.property_id}">
            <td>${p.property_id}</td>
            ...
            <td>
                ...
                <button class="btn btn-outline-danger btn-sm delete-btn" data-id="${p.property_id}">
...
//delete with showing sweetalert
...
const baseEditUrl = "{{ url('/admin/propertyedit') }}"; // ✅ ADD THIS LINE
...
/* Stat cards gradient and animation */
.stat-card {
    border: none;
    position: relative;
    overflow: hidden;
